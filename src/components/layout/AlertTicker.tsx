...
import { AlertTriangle, Info, Siren, CheckCircle2 } from "lucide-react";
import { cn } from "@/lib/utils";

function niveauStyle(n: Alert["niveau"]) {
  switch (n) {
    case "kritiek":
      return { Icon: Siren, cls: "text-critical" };
    case "waarschuwing":
      return { Icon: AlertTriangle, cls: "text-warning" };
    case "succes":
      return { Icon: CheckCircle2, cls: "text-success" };
    default:
      return { Icon: Info, cls: "text-info" };
  }
}
...
export function AlertTicker() {
  ...
  return (
    <div className="flex items-center gap-3 border-t border-border bg-card/60 px-3 py-2">
      <div className="flex shrink-0 items-center gap-1.5 rounded bg-primary/15 px-2 py-1 text-[11px] font-bold uppercase tracking-wide text-primary">

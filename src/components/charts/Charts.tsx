...
const AXIS = { stroke: "#5a6573", fontSize: 11, tickLine: false };
const GRID = "#21262d";
...
export function BarsChart({
  ...
  unit?: string;
  stacked?: boolean;
  colorByKey?: string; // when set, each bar cell uses row[colorByKey] as fill
}) {
  return (
    <ResponsiveContainer width="100%" height={height}>
      <BarChart data={data} margin={{ top: 8, right: 12, left: -16, bottom: 0 }}>
        <CartesianGrid stroke={GRID} vertical={false} />
        <XAxis dataKey={xKey} {...AXIS} interval={0} minTickGap={4} />
        <YAxis {...AXIS} width={44} />
        ...
        {bars.map((b) => (
          ...
          >
            {colorByKey &&
              data.map((row, i) => (
                <Cell key={i} fill={(row[colorByKey] as string) ?? b.color} />
              ))}
          </Bar>
        ))}
      </BarChart>
